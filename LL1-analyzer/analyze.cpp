...
void analyze(const char* sentence, int len) {
	...
	for (int i = 0;i < len;) {
		...
		if (top == '$') {
			if (str[i] == '$') {
				printCurrentStatus(stk, str + i, string("接受"));
				cout << "分析成功！" << endl;
				break;
			}
			else {
				...
				cerr << "分析栈栈顶为'$'，但是输入尚未结束。正在退出..." << endl;
				exit(EXIT_FAILURE);
			}
		}
		// 如果栈顶不是 '$'
		...
		if (current_type == Terminator) {
			if (top == str[i]) {
				...
				//打印 当前栈中内容 + 剩余输入 + 分析动作
				printCurrentStatus(stk, str + i, string("错误！"));
				cerr << "分析栈栈顶为终结符 '" << top << "'，但是输入符号为 '" << str[i] << "'。正在退出..." << endl;
				exit(EXIT_FAILURE);
			}
		}
			...
			if (g_index != -1) {
				...
				//打印 当前栈中内容 + 剩余输入 + 分析动作
				printCurrentStatus(stk, str + i, string("错误！"));
				cerr << "分析表 table[" << top << "][" << str[i] << "] 项为空。正在退出..." << endl;
				exit(EXIT_FAILURE);
			}
		}
		else {
			cerr << "解析错误！正在中止程序..." << endl;
			exit(EXIT_FAILURE);
		}
	}
